...
- Gate D 的本地部分已通过：`build:toy`、项目验包器与官方 `toy_doctor.py` 全部通过；`toy-out/` 为 **19 文件 / 3,984,105 bytes（3.80 MiB）**。桌面、390×844 与 844×390 做了本地关键入口抽查，未发现文档横向溢出或本项目控制台错误。
- Gate E 仍待真实外部返回值：必须先运行不带 `--yes` 的 Toy 更新取得新预览，独立复核后才能使用本轮用户已明确给出的预授权提交审核；GitHub 和飞书结果随后记录。
